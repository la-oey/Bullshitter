...
noToM.lapseRate <- function(matr, weight){
  ...
  regOffDiag <- sweep(offDiag, MARGIN=c(2,3), colSums(offDiag),`/`)
  weightedOffDiag <- array(1-diag(11), dim=c(11,11,6))*(weight*regOffDiag + (1-weight)*1/10)
  onDiag <- array(diag(11), dim=c(11,11,6))*predS
  weightedS <- sweep(weightedOffDiag, MARGIN=c(2,3), (1-apply(onDiag, MARGIN=3, diag)),`*`) + onDiag # lapse rate on lies only
  return(weightedS)
}
