...
function clickConsent() {
    ...
    $('.instructLiePenal').html(expt.liePenalty);
    $('.instructLieReward').html(expt.lieReward);
    $('#instructBSProp').html(trial.compDetect * 100);
...
function clickPostPractice(){
    ...
    expt.pseudo = distributePseudo(100, 0, 10);
    ...
    if(!trial.callBS){
        ...
        if(trial.roleCurrent == 'bullshitter'){
            ...
            if(trial.reportedDrawn == trial.drawnRed){
                ...
                trial.oppTrialScore = expt.marblesSampled - trial.reportedDrawn - 5; //opponent gets 10 - points as reported w/ -5 penalty

            } else{
                trial.callBStxt = trial.callBStxt + "You were <b>lying</b>.<br>The true answer was <b>" + trial.drawnRed + "</b>.";
                expt.stat.lie += 1;
                expt.stat.lie_BS += 1;
                trial.playerTrialScore = -expt.liePenalty; //player gets -x points
                trial.oppTrialScore = expt.marblesSampled - trial.reportedDrawn + expt.lieReward; //opponent gets +x points
            }
        } else{
            ...
            if(trial.reportedDrawn == trial.drawnRed){
                ...
                trial.callBStxt = trial.callBStxt + "Your opponent was <b>lying</b>.<br>The true answer was <b>" + trial.drawnRed + "</b>.";
                expt.stat.BS_lie += 1;
                trial.playerTrialScore = expt.marblesSampled - trial.reportedDrawn + expt.lieReward; //player gets +x points
                trial.oppTrialScore = -expt.liePenalty; //opponent gets -x points
            }
        }
    }
...
function trialDone() {
    ...
    } else if(expt.stat.playerTotalScore >= expt.goalScore){
...
function recordData(){
    trialData.push({
        ...
        liePenalty: expt.liePenalty,
        lieReward: expt.lieReward,
