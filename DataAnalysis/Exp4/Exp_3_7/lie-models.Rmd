...
### simulated recursive ToM models

```{r}
# get recursive liar behavior
# numSims = 1000
# 
# recursiveToM_liar <- data.frame(role=factor(),
#                       p=numeric(), 
#                       expt=factor(),
#                       mor=numeric(),
#                       k=numeric(),
...
#     for(k in c(4,5)){
#       Expt = k
#       sim.L <- replicate(numSims, p.L_ksay.k.r(i, recurse.D(0.1, i, rep(0.1,11))))
#       
#       recursiveToM_liar <- bind_rows(recursiveToM_liar,
#                                 data.frame(role="Liar", p=i, expt=paste0("expt",k), 
#                                       mor=j,
#                                       k = rep(rep(0:10, each=11),numSims),
#                                       ksay = rep(0:10, 11*numSims),
#                                       val=as.vector(sim.L),
#                                       n=rep(1:numSims, each=11)))
#     }
#   }
# }

#write.csv(recursiveToM_liar, "lie-models/recursiveToM_liar.csv")
# recursiveToM_liar %>%
#   filter(mor == 0) %>%
#   group_by(p, expt, k, ksay) %>%
#   summarise(mean = mean(val)) %>%
#   ggplot(aes(x=k, y=ksay, fill=mean)) +
#   geom_tile(stat="identity") +
#   facet_grid(expt ~ p)
```
...
### simulated recursive ToM

```{r}
# get recursive detector behavior
# numSims = 1000
# 
# recursiveToM_detector <- data.frame(role=factor(),
#                       p=numeric(), 
#                       expt=factor(),
#                       mor=numeric(),
#                       ks=numeric(), # ksay in Detector
#                       val=numeric(),
#                       n=numeric()) # prop in Detector
# 
# for(i in c(0.2, 0.5, 0.8)){
#   for(j in c(0,8,20)){
#     moral = j
...
#       
#       recursiveToM_detector <- bind_rows(recursiveToM_detector,
...
#write.csv(recursiveToM_detector, "lie-models/recursiveToM_detector.csv")



# recursiveToM_detector %>%
#   filter(mor == 0) %>%
#   ggplot(aes(x=ks, y=val, colour=as.factor(p))) +
#   geom_line(stat="summary", lty="dashed") +
#   geom_line(data=full.counts.detect.p, aes(x=reportedDrawn, y=prop, colour=as.factor(probabilityRed))) +
