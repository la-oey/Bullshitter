...
for(i in c(0.2, 0.5, 0.8)){
  for(j in c(0.1, 0.25, 0.5)){
    ...
    se <- prop * (1-prop) / sqrt(numSims)
    df.unif <- bind_rows(df.unif, data.frame(role="Detector2", p=i, decay=j, ks=0:10, n=numSims, val=prop, se=se))
  }
}
...
lie.recurse <- df.unif %>%
  ...
  geom_abline(size=2, intercept = 0, slope = 1, colour="forestgreen", linetype=2) +
...
detectProb.recurse <- df.unif %>%
  filter(role=="Detector2", decay==0.1) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(size=1, alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
<!-- ```{r} -->
<!-- df0.5 %>% -->
<!--   ggplot(aes(x=drawnRed, y=reportedDrawn, fill=proportion)) + -->
<!--   geom_bin2d(stat="identity") -->
<!--  # lie results -->

<!-- bs.final %>% -->
<!--   filter(roleCurrent == "bullshitter", probabilityRed == 0.5) %>% -->
<!--   arrange(drawnRed, reportedDrawn) %>% -->
<!--   ggplot(aes(x=drawnRed, y=reportedDrawn)) + -->
<!--   geom_density2d() + -->
<!--   scale_x_continuous(limits=c(0,10)) + -->
<!--   scale_y_continuous(limits=c(0,10)) + -->
<!--   facet_wrap(~drawnRed != reportedDrawn) -->

<!-- resultsProbBS -->
<!-- truek = 5 -->
<!-- mapply(u.L, 0:10, lie=0:10!=truek, BS=TRUE) * resultsProbBS + mapply(u.L, 0:10, lie=0:10!=truek, BS=FALSE) * (1-resultsProbBS) -->
<!-- EV.L_ksay.k.r(10, 0:10, 0.5, resultsProbBS) -->
<!-- round(mapply(function(i) EV.L_ksay.k.r(i, 0:10, 0.5, resultsProbBS), 0:10),4) -->
<!-- P.KSAY.K <- round(apply(mapply(function(i) EV.L_ksay.k.r(i, 0:10, 0.5, resultsProbBS), 0:10), 2, softmax),4) -->


<!-- sum(mat0.5[1,]*P.K[1,]*LIE[1,])/sum(mat0.5[1,]*P.K[1,]) -->
<!-- lie0.5 -->

<!-- P.K <- matrix(rep(p.k(0:numMarbles, 0.8), each=numMarbles+1), nrow=numMarbles+1) -->
<!-- EV.lie <- mapply(function(i) EV.L_ksay.k.r(i, 0:numMarbles, 0.5, resultsProbBS), 0:numMarbles) -->
<!-- round(apply(EV.lie,2,softmax),4) # predicted lie from results -->
<!-- EV.L_ksay.k.r(9, 0:numMarbles, 0.5, resultsProbBS) -->


<!-- EV.D_bs.ksay.r(0:10, 0.5, TRUE, lie0.5) -->
<!-- mapply(p.D_bs.ksay.r, 0:10, 0.8, lie0.8) -->

<!-- rowSums(P.K*mat0.5*LIE)/rowSums(P.K*mat0.5) # -->
<!-- # colSums(P.K*P.KSAY.K*LIE)/colSums(P.K*P.KSAY.K) this isn't correct -->
<!-- rowSums(P.K*P.KSAY.K*LIE)/rowSums(P.K*P.KSAY.K) -->

<!-- round(P.K,3) -->

<!-- resultsProbBS <- bsPropsDF %>% -->
...
for(i in c(0.2, 0.5, 0.8)){
  for(j in c(0, 20)){
    ...
    prop <- colMeans(sim.D)
    se <- prop * (1-prop) / sqrt(numSims)
    df.unif <- bind_rows(df.unif, data.frame(role="Detector", p=i, mor=j, ks=0:10, n=numSims, val=prop, se=se))
    
    sim.L <- t(replicate(numSims, exp.ksay(i, recurse.D(0.1, i, rep(0.1,11)))))
    expLie <- colMeans(sim.L)
    se <- apply(sim.L, 2, sd) / sqrt(numSims)
    df.unif <- bind_rows(df.unif, data.frame(role="Liar", p=i, mor=j, ks=0:10, n=numSims, val=expLie, se=se))
    
    sim.D2 <- t(replicate(numSims, recurse.L(0.1, i, rep(0.1,11))))
    prop <- colMeans(sim.D2)
    se <- prop * (1-prop) / sqrt(numSims)
    df.unif <- bind_rows(df.unif, data.frame(role="Detector2", p=i, mor=j, ks=0:10, n=numSims, val=prop, se=se))
  }
}
...
lie.recurse.moral <- df.unif %>%
  filter(role=="Liar", mor==20) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_abline(size=3, intercept = 0, slope = 1, colour="forestgreen", linetype=2) +
  geom_line(size=1.5, alpha=0.8) +
  ggtitle("With Morality") +
  scale_x_continuous("True Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("Expected Reported", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
  scale_colour_manual(values=condColors.diff) +
  theme_minimal() +
  theme(legend.position = "none",
        ...
        axis.text = element_text(size=24))
lie.recurse.moral
ggsave("img/modelRecurse_lie_moral.png", lie.recurse.moral, height=8, width=7.5)

detect.recurse.moral <- df.unif %>%
  filter(role=="Detector", mor==20) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(size=1.5, alpha=0.8) +
  ggtitle("With Morality") +
...
detectProb.recurse.moral <- df.unif %>%
  filter(role=="Detector2", mor==20) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(size=1.5, alpha=0.8) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=28),
        axis.text = element_text(size=24))
detectProb.recurse.moral
ggsave("img/modelRecurse_detect2_moral.png", detectProb.recurse.moral, height=8, width=7.5)
```
...
detect.respondToHumanData <- data.frame(p=c(rep(0.2,11), rep(0.5,11), rep(0.8,11)),
  ...
  geom_line(size=1.5, alpha=0.8) +
  ggtitle("Response to Human Lying") +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("Expected P(BS)", limits=c(0,1)) +
  scale_colour_manual(values=condColors.diff)+
  theme_minimal() +
...
# Fix response to idiot liar and detector!!!!

```{r}
data.frame(p=c(rep(0.2,11), rep(0.5,11), rep(0.8,11)),
           ksay=rep(0:10,3),
           bs=c(mapply(p.D_bs.ksay.r, 0:numMarbles, 0.2, rep(0.1,11)),
                mapply(p.D_bs.ksay.r, 0:numMarbles, 0.5, rep(0.1,11)),
                mapply(p.D_bs.ksay.r, 0:numMarbles, 0.8, rep(0.1,11))))
           
detectIdiot <- mapply(p.D_bs.ksay.r, 0:numMarbles, 0.8, lie0.8)
```
...
lie.respondToHumanData <- bsPropsDF %>%
  ...
  geom_abline(intercept = 0, slope = 1, colour="forestgreen", linetype=2, size=3) +
  geom_line(size=1.5, alpha=0.8) +
  ggtitle("Response to Human Detecting") +
  scale_x_continuous("True Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("Expected Reported", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
  scale_colour_manual(values=condColors.diff) +
  theme_minimal() +
...
lie.results <- bs.final %>%
  ...
  theme(legend.position = "none",
        ...
        axis.text = element_text(size=24))
lie.results
ggsave("img/results_lie.png", lie.results, height=8, width=7.5)

detect.results <- bs.final %>%
  filter(roleCurrent == "bullshitDetector") %>%
  mutate(probabilityRed.txt = paste("p =", probabilityRed)) %>%
  group_by(probabilityRed.txt, reportedDrawn) %>%
  summarise(prop = (sum(callBS == "True")+1)/(n()+2),
  ...
  theme(legend.position = "none",
        ...
        axis.text = element_text(size=24))
detect.results
ggsave("img/results_detect.png", detect.results, height=8, width=7.5)
```
...
```{r}
modelComp <- df.unif %>%
  filter(decay==0.25, alpha==0.4, beta==0.8) %>%
  mutate(k = ks,
         mean.model = val,
         se.model = se) %>%
...
## Liar
liar.decay <- df.unif %>%
  filter(role=="Liar", alpha==0.4, beta==0.8) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
...
detector.decay <- df.unif %>%
  filter(role=="Detector", alpha==0.4, beta==0.8) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
detector2.decay <- df.unif %>%
  filter(role=="Detector2", alpha==0.4, beta==0.8) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
ggsave("img/vary_decay.png", grid.arrange(liar.decay, detector.decay, detector2.decay, nrow=3))
```
...
## Liar
liar.alpha <- df.unif %>%
  filter(role=="Liar", decay==0.1, beta==0.8) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_abline(intercept = 0, slope = 1, colour="forestgreen", linetype=2) +
  geom_line(alpha=0.9) +
...
detector.alpha <- df.unif %>%
  filter(role=="Detector", decay==0.1, beta==0.8) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
detector2.alpha <- df.unif %>%
  filter(role=="Detector2", decay==0.1, beta==0.8) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
ggsave("img/vary_alpha.png", grid.arrange(liar.alpha, detector.alpha, detector2.alpha, nrow=3))
```
...
## Liar
liar.beta <- df.unif %>%
  filter(role=="Liar", decay==0.25, alpha==0.4) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_abline(intercept = 0, slope = 1, colour="forestgreen", linetype=2) +
  geom_line(alpha=0.9) +
...
detector.beta <- df.unif %>%
  filter(role=="Detector", decay==0.25, alpha==0.4) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
detector2.beta <- df.unif %>%
  filter(role=="Detector2", decay==0.25, alpha==0.4) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(alpha=0.9) +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
...
ggsave("img/vary_beta.png", grid.arrange(liar.beta, detector.beta, detector2.beta, nrow=3))
```
